...
require 'rspec'
result_handler = ResultHandler.new(x2t.version, 'Docxf to Dotx')
...
describe 'Conversion docxf files to dotx' do
  before do
  ...
  files.each do |file|
    it File.basename(file) do
      ...
      @file_data = x2t.convert("#{@tmp_dir}/#{File.basename(file)}", :dotx)
