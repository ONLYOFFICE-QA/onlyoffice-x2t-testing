...
require 'rspec'
result_handler = ResultHandler.new(x2t.version, 'Xlsx to Xltm')
...
describe 'Conversion xlsx files to xltm' do
  before do
  ...
  files.each do |file|
    it File.basename(file) do
      ...
      @file_data = x2t.convert("#{@tmp_dir}/#{File.basename(file)}", :xltm)
