...
require 'rspec'
result_handler = ResultHandler.new(x2t.version, 'Pptx to Pptm')
...
describe 'Conversion pptx files to pptm' do
  before do
  ...
  files.each do |file|
    it File.basename(file) do
      ...
      @file_data = x2t.convert("#{@tmp_dir}/#{File.basename(file)}", :pptm)
