...
require 'rspec'
result_handler = ResultHandler.new(x2t.version, 'Pptx to Ppsx')
...
describe 'Conversion pptx files to ppsx' do
  before do
  ...
  files.each do |file|
    it File.basename(file) do
      ...
      @file_data = x2t.convert("#{@tmp_dir}/#{File.basename(file)}", :ppsx)
